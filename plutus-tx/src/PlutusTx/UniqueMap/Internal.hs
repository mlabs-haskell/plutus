...
{- | An association data structure, suitable for on-chain use, which preserves
 ...
 testing helper to allow us to define instances needed for testing
...
newtype UniqueMap k v
  ...
  deriving newtype (Eq, Ord, Prelude.Eq)
...
{-
    where
      go :: [(BuiltinData, BuiltinData)] -> UniqueMap k v
      go = \case
        [] -> empty
        ((k', v') : ps) ->
          let key = unsafeFromBuiltinData k'
              value = unsafeFromBuiltinData v'
           in snd . foldr go2 (key, singleton key value) $ ps
      go2 ::
      ...
      go2 (k', v') (lastMax, acc) =
        let key = unsafeFromBuiltinData k'
         in case compare lastMax key of
              LT -> (key, insert key (unsafeFromBuiltinData v') acc)
              _ -> error ()
-}
  ...
  parseJSON = withArray "UniqueMap" go
    where
      ...
      go v = case Vector.uncons v of
        Nothing -> Prelude.pure empty
        Just (val, vals) -> do
          (key, value) <- parseJSON val
          snd Prelude.<$> foldM go2 (key, singleton key value) vals
      go2 :: (k, UniqueMap k v) -> Value -> Parser (k, UniqueMap k v)
      go2 (lastMax, acc) val = do
        (key, value) <- parseJSON val
        case compare lastMax key of
          LT -> Prelude.pure (key, insert key value acc)
          EQ -> Prelude.fail "Duplicate keys"
          GT -> Prelude.fail "Keys are out of order"
...
-}
{-# INLINEABLE empty #-}
empty :: forall k v. UniqueMap k v
empty = UniqueMap []
...
singleton :: forall k v. k -> v -> UniqueMap k v
...
-}
{-# INLINEABLE fromList #-}
fromList :: Ord k => [(k, v)] -> UniqueMap k v
...
-}
{-# INLINEABLE lookup #-}
lookup :: forall k v. Ord k => k -> UniqueMap k v -> Maybe v
...
-}
{-# INLINEABLE findMin #-}
findMin :: UniqueMap k v -> Maybe (k, v)
...
-}
{-# INLINEABLE findMax #-}
findMax :: UniqueMap k v -> Maybe (k, v)
 ...
 * The value for @key@ is the same in both @m@ and @m'@.
 ...
 /Complexity: \(\Theta( )\)
-}
inclusion :: forall k v. (Ord k, Eq v) => UniqueMap k v -> UniqueMap k v -> Inclusion
...
-}
{-# INLINEABLE insert #-}
insert :: forall k v. Ord k => k -> v -> UniqueMap k v -> UniqueMap k v
...
-}
{-# INLINEABLE delete #-}
delete :: forall k v. Ord k => k -> UniqueMap k v -> UniqueMap k v
...
-}
{-# INLINEABLE alter #-}
alter :: forall k v. Ord k => k -> (Maybe v -> Maybe v) -> UniqueMap k v -> UniqueMap k v
...
{- | Given a transformation function for the value that uses the key, transform
 ...
 /Complexity:/ \(\Theta(n)\) assuming function is \(\Theta(1)\)
-}
mapWithKey :: forall k v u. (k -> v -> u) -> UniqueMap k v -> UniqueMap k u
...
-}
{-# INLINEABLE mapMaybe #-}
mapMaybe :: (v -> Maybe u) -> UniqueMap k v -> UniqueMap k u
...
mapMaybeWithKey :: forall k v u. (k -> v -> Maybe u) -> UniqueMap k v -> UniqueMap k u
...
-}
{-# INLINEABLE union #-}
union :: forall k v u. Ord k => UniqueMap k v -> UniqueMap k u -> UniqueMap k (These v u)
...
unionWith :: forall k v. Ord k => (v -> v -> v) -> UniqueMap k v -> UniqueMap k v -> UniqueMap k v
